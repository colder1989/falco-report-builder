...
export const InvestigationDashboard = () => {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-amber-50 via-yellow-50 to-amber-100 transition-all duration-1000 ${isLoaded ? 'fade-in' : 'opacity-0'}`}>
      {/* Enhanced Header */}
      <header className="bg-amber-50/90 backdrop-blur-md border-b border-amber-200/60 shadow-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4 slide-in-up" style={{ animationDelay: '0.2s' }}>
              <div className="flex items-center space-x-2 bg-amber-100 border border-amber-200 px-3 py-2 rounded-lg">
                {getStatusIcon()}
                <span className="text-sm font-medium">{getStatusText()}</span>
              </div>
              <Badge variant="outline" className="bg-amber-50 border-amber-300 text-slate-700">
                {stats.percentage}% Completato
              </Badge>
              <Button
                variant="outline"
                onClick={() => setShowPreview(!showPreview)}
                className="flex items-center space-x-2 floating-button bg-amber-50 border-amber-300 hover:bg-amber-100"
              >
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="pt-4 space-y-3">
                    <Button 
                      ...
                      size="sm" 
                      onClick={resetData}
                      className="w-full floating-button hover:bg-amber-100 bg-amber-50 border-amber-300"
                    >
                ...
                <CardContent className="space-y-3">
                  <div className="text-sm space-y-3">
                    {[
                      ...
                      <div 
                        key={index} 
                        className="flex items-center space-x-3 p-2 rounded-lg hover:bg-amber-100 transition-colors duration-200"
                      >
